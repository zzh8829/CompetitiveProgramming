...
double dfs(Board* orig, int s)
{
  if(s == 1)
  {
    return -0.510066 * orig->height() + 
            0 * 0.760666 * orig->nclear() -  
            0 * 0.35663 * orig->holes() - 
            0 * 0.184483 * orig->bumps();
  }
  int maxscore = -99999;
  for(int r = 0; r < 4; r++)
  {
    orig->block->rotate();
    Board *board = orig->copy();
...
int main(int argc, char** argv) {
  ...
  ofstream of("log.txt", ios_base::app);

  double maxscore = -99999;
  vector<string> maxmoves;

  Board * orig = new Board(state);

  for(int r = 0; r < 4; r++)
  {
    if(r) orig->block->rotate();
    ...
    while(board->check(*(board->block)))
    {
      ...
      if(score > maxscore)
      {
        of << "MAX: " << score << " " << r << " " << am << endl;
        b2->print(of);
        maxscore = score;
        maxmoves.clear();
        maxmoves.push_back("down");
        maxmoves.push_back("down");
        for(int k = 0; k < r; k++)
          maxmoves.push_back("rotate");
        for(int k = 0; k < int(abs(am)); k++)
        {
          if(am < 0)
            maxmoves.push_back("left");
          if(am > 0)
            maxmoves.push_back("right");
        }
      }
  ...
  for (int i = 0; i < maxmoves.size() - 1; i++) {
    cout << maxmoves[i] << endl;
  }
